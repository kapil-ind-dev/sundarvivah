...
<div class="card-body">
     ...
     <form action="{{ route('member.basic_info_update', $member->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group row">
                <div class="col-md-6">
                    <label for="member_id" >{{translate("Member Id")}}
                        <!--<span class="text-danger">*</span>-->
                    </label>
                    <input type="text" name="member_id" value="{{ $member->member_id }}" class="form-control" placeholder="{{translate('Member Id')}}">
                    @error('member_id')
                        <small class="form-text text-danger">{{ $message }}</small>
                    @enderror
                </div>
                <div class="col-md-6">
                    <label for="blue_tick" >{{translate('Blue Tick')}}
                        <!--<span class="text-danger">*</span>-->
                    </label>
                    <select class="form-control aiz-selectpicker" name="blue_tick">
                        <option value="">{{translate('Select One')}}</option>
                        <option value="1" @if($member->blue_tick ==  1) selected @endif >{{translate('Yes')}}</option>
                        <option value="2" @if($member->blue_tick ==  2) selected @endif >{{translate('No')}}</option>
                        @error('blue_tick')
                            <small class="form-text text-danger">{{ $message }}</small>
                        @enderror
                    </select>
