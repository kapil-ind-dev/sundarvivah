<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route('address.update', $member->id) }}" method="POST">
            ...
            <div class="form-group row">
                <div class="col-md-12">
                    <label for="full_address">{{translate('Full Address')}}</label>
                    <input type="text" name="full_address" value="{{$full_address}}" class="form-control" placeholder="{{translate('Full Address')}}" >
                    @error('full_address')
                        <small class="form-text text-danger">{{ $message }}</small>
                    @enderror
                </div>
