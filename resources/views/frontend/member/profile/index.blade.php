...
@section('panel_content')
    ...
    @if(get_setting('member_family_information_section') == 'on')
      @include('frontend.member.profile.family_information.index')
    @endif
    ...
    @php
        ...
        $disability_accept      = $member->partner_expectations->disability_accept ?? "";
...
<script type="text/javascript">
    ...
    //  Family information Add edit , status change
    function family_information_add_modal(id){
       $.post('{{ route('family_information.create') }}',{_token:'{{ @csrf_token() }}', id:id}, function(data){
           $('.create_edit_modal_content').html(data);
           $('.create_edit_modal').modal('show');
       });
    }

    function family_information_edit_modal(id){
        $.post('{{ route('family_information.edit') }}',{_token:'{{ @csrf_token() }}', id:id}, function(data){
            $('.create_edit_modal_content').html(data);
            $('.create_edit_modal').modal('show');
        });
    }

    function update_family_information_status(el) {
        if (el.checked) {
            var status = 1;
        } else {
            var status = 0;
        }
        $.post('{{ route('family_information.update_family_information_status') }}', {
            _token: '{{ csrf_token() }}',
            id: el.value,
            status: status
        }, function (data) {
   ...
   function getProfession(employedin_id) {
    ...
    // Disable the select during the AJAX request
    $select.prop('disabled', true);

    $.post('{{ route('get-professions') }}', {
        _token: '{{ csrf_token() }}',
    ...
    .done(function (data) {
        if (Array.isArray(data) && data.length > 0) {
            ...
            $select.append('<option value="">Select a profession</option>');
            
            // Iterate over the response data and add options to the select element
            $.each(data, function (index, item) {
                $select.append('<option value="' + item.id + '">' + item.title + '</option>');
            });
        } else {
            // Clear existing options and add a 'No professions found' option
            $select.empty().append('<option value="">No professions found</option>');
            AIZ.plugins.notify('danger', 'No professions found');
        }
    })
    .fail(function () {
        // Clear existing options and add a 'No professions found' option
    ...
    .always(function() {
        // Re-enable the select when done
