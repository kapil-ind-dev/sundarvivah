<form action="{{ route('career.update', $career->id) }}" method="POST">
    ...
    <div class="modal-body">
        ...
        <div class="form-group row">
            ...
            <div class="col-md-9">
                <input type="text" name="job_location" value="{{$career->job_location}}" class="form-control" placeholder="{{translate('Job Location')}}" required>
            </div>
        </div>
        <div class="form-group row">
            <label class="col-md-3 col-form-label">{{translate('Income')}}</label>
            <div class="col-md-9">
                <input type="text" name="income" value="{{$career->income}}"  placeholder="{{ translate('Income') }}" class="form-control">
            </div>
        </div>
    </div>
