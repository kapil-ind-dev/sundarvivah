                            ...
                            <ul class="list-group list-group-raw fs-15 mb-5">
                                <li class="list-group-item py-2 text-line-through">
                                    @if( $package->auto_profile_match == 0 )
                                        ...
                                        <del class="opacity-60">{{ translate('View Auto Profile Matches') }}</del>
                                    @else
                                        <i class="las la-check text-success mr-2"></i>
                                        {{ translate('View Auto Profile Matches') }}
                                    @endif
                                </li>
                                <li class="list-group-item py-2 text-line-through">
                                    @if( $package->unlimited_shortlist == 0 )
                                        <i class="las la-times text-danger mr-2"></i>
                                        <del class="opacity-60">{{ translate('Unlimited Shortlist') }}</del>
                                    @else
                                        <i class="las la-check text-success mr-2"></i>
                                        {{ translate('Unlimited Shortlist') }}
                                    @endif
                                </li>
                                <li class="list-group-item py-2 text-line-through">
                                    @if( $package->fully_unlock_advance_search == 0 )
                                        <i class="las la-times text-danger mr-2"></i>
                                        <del class="opacity-60">{{ translate('Fully unlock advance search') }}</del>
                                    @else
                                        <i class="las la-check text-success mr-2"></i>
                                        {{ translate('Fully Unlock Advance Search') }}
                                    @endif
                                </li>
                                ...
                                <li class="list-group-item py-2">
                                    <i class="las la-check text-success mr-2"></i>
                                    {{ $package->gallery_image_view }} {{ translate('Get Free Profile List in Pdf') }}
                                </li>
                                <!--@endif-->
                                <li class="list-group-item py-2">
                                    <i class="las la-check text-success mr-2"></i>
                                    {{ $package->contact }} {{ translate('View Contact No. with Full Info') }}
                                </li>
                                <li class="list-group-item py-2">
                                    <i class="las la-check text-success mr-2"></i>
                                    {{ $package->photo_gallery }} {{ translate('Girls profiles get free assistance') }}
                                </li>
                                <li class="list-group-item py-2 text-line-through">
                                    @if( $package->free_whats_app_group_for_direct_chat == 0 )
                                        <i class="las la-times text-danger mr-2"></i>
                                        <del class="opacity-60">{{ translate('Free whats app group for direct chat') }}</del>
                                    @else
                                        <i class="las la-check text-success mr-2"></i>
                                        {{ translate('Free Whats App Group for Direct Chat') }}
                                    @endif
                                </li>
